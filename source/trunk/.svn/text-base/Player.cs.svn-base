...
namespace XNAPacMan {
    ...
    public enum State { Start, Normal, Dying, Win };
    ...
    public class Player {
        ...
        string lastbp = null;
        DualShock3 ds3 = new DualShock3().staticJoystick;
        string dir = ""; int it, oldit = -1000;
        ...
        public int Update(GameTime gameTime, frmEvents fe)
        {

            // First, deal with keyboard input. Get only the direction keys.
            Keys[] validKeys = { Keys.Up, Keys.Down, Keys.Left, Keys.Right, Keys.E, Keys.G, Keys.H };
            Keys[] pressedKeys = (from k in Keyboard.GetState().GetPressedKeys()
                                  where validKeys.Contains(k)
                                  select k).ToArray();
            string bp = ds3.GetButton();
            ds3.ReadImmediateData();
            ...
            {
                // E, G and H keys give difficulty feedback
                if (pressedKeys[0] == Keys.E)
                { Level.SetUserInput(Level.UserInput.TooEasy); Constants.LogEasy = true; }
                if (pressedKeys[0] == Keys.G)
                { Level.SetUserInput(Level.UserInput.Good); Constants.LogGood = true; }
                if (pressedKeys[0] == Keys.H)
                { Level.SetUserInput(Level.UserInput.TooHard); Constants.LogHard = true; }
            ...
            else if (bp != null)
            {
                if (state_ == State.Start)
                {
                    if (bp == Constants.ds3Left || bp == Constants.ds3Right)
                    {
                        state_ = State.Normal;
                    }
                    ...
                    switch (bp)
                    {
                        case Constants.ds3Left:
                           TryTurn(Keys.Left);
                            break;
                        case Constants.ds3Right:
                            TryTurn(Keys.Right);
                            break;
            ...
            int[] p = ds3.GetIntensity; // Get the output of the pressure sensitive buttons
            if (bp != null)
            {
                if (bp.Contains(Constants.ds3Up))
                { TryTurn(Keys.Up); dir = "Up"; it = p[0]; }
                else if (bp.Contains(Constants.ds3Down))
                { TryTurn(Keys.Down); dir = "Down"; it = p[1]; }
                else if (bp.Contains(Constants.ds3Left))
                { TryTurn(Keys.Left); dir = "Left"; it = p[2]; }
                else if (bp.Contains(Constants.ds3Right))
                { TryTurn(Keys.Right); dir = "Right"; it = p[3]; }
                else if (bp.Contains(Constants.ds3L1) || bp.Contains(Constants.ds3L2) || bp.Contains(Constants.ds3Square))
                { Level.SetUserInput(Level.UserInput.TooEasy); Constants.LogEasy = true; }
                else if (bp.Contains(Constants.ds3R1) || bp.Contains(Constants.ds3R2) || bp.Contains(Constants.ds3Circle))
                { Level.SetUserInput(Level.UserInput.TooHard); Constants.LogHard = true; }
                else if (bp.Contains(Constants.ds3Triangle))
                { Level.SetUserInput(Level.UserInput.Good); Constants.LogGood = true; }

                fe.UpdateIntensity(p);
                if (it < oldit)
                    it = oldit; // Get the maximum intensity
            }
            
            if (lastbp != bp && bp != null)
            {
                //fe.AddPressureInput(dir, it);
                oldit = -1000;
            }
            fe.UpdateIntensity(p);
            lastbp = bp;
        ...
        public void UpdateFeedback(GameTime gameTime, frmEvents fe)
        {
            ...
            if (bp != null)
            {
                if (bp.Contains(Constants.ds3L1) || bp.Contains(Constants.ds3L2) || bp.Contains(Constants.ds3Square))
                { Level.SetUserInput(Level.UserInput.TooEasy); Constants.LogEasy = true; }
                else if (bp.Contains(Constants.ds3R1) || bp.Contains(Constants.ds3R2) || bp.Contains(Constants.ds3Circle))
                { Level.SetUserInput(Level.UserInput.TooHard); Constants.LogHard = true; }
                else if (bp.Contains(Constants.ds3Triangle))
