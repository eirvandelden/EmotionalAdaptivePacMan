...
namespace XNAPacMan {
    ...
    public class Menu : Microsoft.Xna.Framework.DrawableGameComponent {
        ...
        public override void Initialize() {
            ...
            selection_ = 0;
            if (gameLoop_ == null) {
                items_ = new string[] { "New Game", "High Scores", "Quit Game"};
            }
            else {
                items_ = new string[] { "Resume", "Quit" };
            }
        ...
        public override void Update(GameTime gameTime) {
            ...
            if (Constants.Soundcheck)
            {
                soundBank_.PlayCue("ExtraLife");
                Constants.Soundcheck = false;
            }
            ...
            if (newPressedKeys.Contains(Keys.Down) || bp == Constants.ds3Down) 
            {
                selection_++;
                selection_ %= items_.Length;
                soundBank_.PlayCue("PacMAnEat1");
            }
            else if (newPressedKeys.Contains(Keys.Up) || bp == Constants.ds3Up)
            {
                selection_--;
                selection_ = (selection_ < 0? items_.Length -1 : selection_);
                soundBank_.PlayCue("PacManEat2");
            }
            else if (newPressedKeys.Contains(Keys.Enter) || bp == Constants.ds3Cross)//dsCross
            {
                menuAction();
            }
        ...
        void menuAction() {
            ...
            switch (items_[selection_]) {
                case ("Resume"):
                    ...
                    break;
                case ("New Game") :
                    Game.Components.Add(new LevelSelection(Game));
                    break;
                case ("High Scores"):
                ...
                case ("Quit Game"):
                    try
                    {
                        /*if (gameLoop_.contact)
                        {
                            gameLoop_.Mobi.ReleaseSerialContact();
                            gameLoop_.contact = false;
                        }*/
                    }
                    catch (NullReferenceException)
                    {
                        Console.WriteLine("Succesfully caught and handled NullRefence, Mobi wasn't initiated yet.");
                    }
                    Game.Exit();
                    break;
                case ("Quit"):
